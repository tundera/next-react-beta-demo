...
import MobileNavLink from 'src/components/MobileNavLink'
        ...
        <>
            ...
            <div className="relative flex items-center justify-between h-16">
              ...
              <div className="absolute inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
                <button
                  ...
                  type="button"
                  className="flex items-center justify-center transition-all w-9 h-9 hover:ring-2 ring-gray-300"
                  onClick={() => setTheme(resolvedTheme === 'dark' ? 'light' : 'dark')}
                >
                  {mounted && resolvedTheme === 'dark' ? (
                    <FiSun className="w-5 h-5 text-gray-200 dark:text-gray-800" />
                  ) : (
                    <FiMoon className="w-5 h-5 text-gray-200 dark:text-gray-800" />
                  )}
                </button>
              </div>
            </div>
          ...
          <Disclosure.Panel className="sm:hidden">
            <div className="px-2 pt-2 pb-3 space-y-1">
              ...
              <MobileNavLink href="/teams" text="Teams" />
            </div>
          </Disclosure.Panel>
        </>
      )}
